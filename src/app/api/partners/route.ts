...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const inviteMatch = body.discord_invite_link.match(/discord(?:\.gg|app\.com\/invite)\/([a-zA-Z0-9-]+)/);
    ...
    if (!discordRes.ok) {
        console.error(`[POST /api/partners] - Discord API fetch failed for ${inviteCode}: ${discordRes.status}`);
      return NextResponse.json(
    ...
    if (!guild?.id) {
      console.error(`[POST /api/partners] - Discord API response missing guild data or guild ID for ${inviteCode}`);
      return NextResponse.json(
